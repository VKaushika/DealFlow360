...
export const Navbar: React.FC<{ onNavigate: (page: string, id?: string) => void; activePage: string }> = ({
  onNavigate,
  activePage,
}) => {
  ...
  const personas: Array<{ role: RoleType; label: string; userName: string; badge: string }> = [
    { role: 'SALES_REP', label: 'Sales Rep', userName: 'Sarah Miller', badge: 'Sales' },
    { role: 'SALES_MANAGER', label: 'Sales Manager', userName: 'Marcus Sterling', badge: 'Manager' },
    { role: 'FINANCE_OPS', label: 'Finance & Ops', userName: 'Fiona Vance', badge: 'Finance' },
    { role: 'CUSTOMER', label: 'Customer Portal', userName: 'David Miller', badge: 'Buyer' },
    { role: 'ADMIN', label: 'Administrator', userName: 'Alexander Cross', badge: 'Admin' },
  ];
  ...
  useEffect(() => {
    // Generate live notifications based on backend data
    const fetchNotifs = async () => {
      try {
        ...
        if (appRes.data.success && appRes.data.data) {
          const pending = appRes.data.data.filter((a: any) => a.status === 'PENDING').slice(0, 3);
          pending.forEach((a: any) => {
            items.push({
              id: a._id,
              title: `Pending Approval: ${a.quotationId?.quoteNumber || 'QT-Deal'}`,
              message: `${a.customerId?.companyName || 'Account'} has ${a.riskLevel} risk discount ceiling breach.`,
              type: 'approval',
              timestamp: 'Just now',
        ...
        if (healthRes.data.success && healthRes.data.data) {
          const anomalies = healthRes.data.data.slice(0, 2);
        ...
        if (items.length === 0) {
          items.push({
            ...
            message: 'Deterministic pricing rules and warehouse routing engine active.',
            type: 'info',
            timestamp: 'Now',
  ...
  return (
    <header className="bg-cream-100 border-b border-cream-border text-charcoal-900 px-5 sm:px-8 py-3 sticky top-0 z-40">
      <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-4">
        {/* Left: Brand Logo, Back Arrow & Breadcrumb */}
        <div className="flex items-center gap-4 sm:gap-6">
          <div
            className="flex items-center gap-2.5 cursor-pointer group"
            onClick={() => onNavigate(role === 'CUSTOMER' ? 'portal' : 'dashboard')}
          >
          ...
          {activePage !== 'dashboard' && activePage !== 'portal' && (
            <button
          ...
          <div className="hidden sm:flex items-center gap-2 text-xs font-mono text-charcoal-400 uppercase tracking-wider pl-3 border-l border-cream-border">
            <span>Workspace</span>
            <span>/</span>
            <span className="text-charcoal-800 font-bold">{pageTitleMap[activePage] || 'Dashboard'}</span>
          </div>
        </div>

        {/* Center/Right: Role Switcher Segment & User Profile */}
        <div className="flex items-center gap-3">
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (p.role === 'CUSTOMER') onNavigate('portal');
                  }}
                ...
                >
                  <span className={`w-1.5 h-1.5 rounded-full ${isActive ? 'bg-charcoal-950' : 'bg-charcoal-400'}`} />
                  <span>{p.label}</span>
                </button>
              );
            })}
          ...
          <div className="lg:hidden">
            <select
              ...
              onChange={(e) => {
                ...
                if (newRole === 'CUSTOMER') onNavigate('portal');
              }}
              className="bg-white border border-cream-border text-xs text-charcoal-800 font-bold rounded-xl px-2.5 py-1.5 focus:outline-none"
            >
              {personas.map((p) => (
          ...
          {/* Login / RBAC Security Portal Quick Button */}
          <button
          ...
          >
            ...
            <span className="hidden sm:inline">RBAC Login</span>
          </button>

          {/* Functional Notification Bell with Popover Drawer */}
              ...
              <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white border border-cream-border rounded-2xl shadow-2xl z-50 p-4 space-y-3 animate-fade-in">
                <div className="flex items-center justify-between pb-2 border-b border-cream-border">
                  <div className="flex items-center gap-2">
                    ...
                    <h4 className="font-bold text-xs text-charcoal-900">Active Notifications</h4>
                ...
                <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
                  {notifications.map((n) => (
                    <div
                      ...
                      onClick={() => {
                        setShowNotifications(false);
                        onNavigate(n.targetPage, n.targetId);
                      }}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] font-mono text-charcoal-400">{n.timestamp}</span>
                      </div>
                      <p className="text-[11px] text-charcoal-600 font-medium leading-snug">{n.message}</p>
                    </div>
                  ))}
                </div>
          ...
          >
            <div className="text-right hidden sm:block">
              <p className="text-xs font-bold text-charcoal-900 leading-tight group-hover:text-brand-600 transition-colors">
                {user?.name || currentPersona.userName}
              </p>
